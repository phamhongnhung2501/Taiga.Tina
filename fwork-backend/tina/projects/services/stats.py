...
def get_stats_for_project(project):
    ...
    user_stories = apps.get_model('userstories', 'UserStory').objects.filter(
        project_id = project.id
    )

    #Get numbers of userstories and how many on each status
    project._total_user_stories = user_stories.count()
    project._userstories_new = user_stories.filter(status_id = 1).count()
    project._userstories_ready = user_stories.filter(status_id = 2).count()
    project._userstories_in_progress = user_stories.filter(status_id = 3).count()
    project._userstories_ready_for_test = user_stories.filter(status_id = 4).count()
    project._userstories_done = user_stories.filter(status_id = 5).count()
    project._userstories_archived = user_stories.filter(status_id = 6).count()
    ...
    project_stats = {
        ...
        'speed': speed,
        'userstories_new': project._userstories_new,
        'userstories_ready': project._userstories_ready,
        'userstories_in_progress': project._userstories_in_progress,
        'userstories_ready_for_test': project._userstories_ready_for_test,
        'userstories_done': project._userstories_done,
        'userstories_archived': project._userstories_archived,
        'total_user_stories': project._total_user_stories
    }
