...
from tina.base.fields import Field, MethodField, I18NField

from tina.permissions import services as permissions_services
from tina.users.services import get_photo_url, get_user_photo_url
from tina.users.gravatar import get_gravatar_id, get_user_gravatar_id
from tina.users.serializers import UserBasicInfoSerializer

from tina.permissions.services import calculate_permissions
from tina.permissions.services import is_project_admin, is_project_owner
...
class ProjectLightSerializer(serializers.LightSerializer):
    ...
    userstories_new = MethodField()
    userstories_ready = MethodField()
    userstories_in_progress = MethodField()
    userstories_ready_for_test = MethodField()
    userstories_done = MethodField()
    userstories_archived = MethodField()
    ...
    def get_userstories_new(self, obj):
        assert hasattr(obj, "userstories_new_attr"), "instance must have a userstories_new_attr attribute"
        if obj.userstories_new_attr is None:
            return 0

        userstories = 0
        for userstory in obj.userstories_new_attr:
            if userstory["id"] is not None:
                userstories += 1

        return userstories

    def get_userstories_ready(self, obj):
        assert hasattr(obj, "userstories_ready_attr"), "instance must have a userstories_ready_attr attribute"
        if obj.userstories_ready_attr is None:
            return 0

        userstories = 0
        for userstory in obj.userstories_ready_attr:
            if userstory["id"] is not None:
                userstories += 1

        return userstories

    def get_userstories_in_progress(self, obj):
        assert hasattr(obj, "userstories_in_progress_attr"), "instance must have a userstories_in_progress_attr attribute"
        if obj.userstories_in_progress_attr is None:
            return 0

        userstories = 0
        for userstory in obj.userstories_in_progress_attr:
            if userstory["id"] is not None:
                userstories += 1

        return userstories

    def get_userstories_ready_for_test(self, obj):
        assert hasattr(obj, "userstories_ready_for_test_attr"), "instance must have a userstories_ready_for_test_attr attribute"
        if obj.userstories_ready_for_test_attr is None:
            return 0

        userstories = 0
        for userstory in obj.userstories_ready_for_test_attr:
            if userstory["id"] is not None:
                userstories += 1

        return userstories

    def get_userstories_done(self, obj):
        assert hasattr(obj, "userstories_done_attr"), "instance must have a userstories_done_attr attribute"
        if obj.userstories_done_attr is None:
            return 0

        userstories = 0
        for userstory in obj.userstories_done_attr:
            if userstory["id"] is not None:
                userstories += 1

        return userstories

    def get_userstories_archived(self, obj):
        assert hasattr(obj, "userstories_archived_attr"), "instance must have a userstories_archived_attr attribute"
        if obj.userstories_archived_attr is None:
            return 0

        userstories = 0
        for userstory in obj.userstories_archived_attr:
            if userstory["id"] is not None:
                userstories += 1
