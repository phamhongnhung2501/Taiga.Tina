...
from tina.base.mails import InlineCSSTemplateMail
...
from tina.projects.history.services import (make_key_from_model_object,
...
def get_mentions(project: object, text: str):
    from tina.mdrender.service import render_and_extract
...
def send_sync_notifications(notification_id):
    ...
    headers = {
        ...
        "List-ID": 'Tina/{project_name} <tina.{project_slug}@{domain}>'.format(**format_args),
...
def get_watched(user_or_id, model):
    ...
    :param user_or_id: :class:`~tina.users.models.User` instance or id.
...
def add_watcher(obj, user):
    ...
    :param obj: Any Django model instance.
    :param user: User adding the watch. :class:`~tina.users.models.User` instance.
    """
    obj_type = apps.get_model("contenttypes", "ContentType").objects.get_for_model(obj)
...
def remove_watcher(obj, user):
    ...
    :param obj: Any Django model instance.
    :param user: User removing the watch. :class:`~tina.users.models.User` instance.
    """
    obj_type = apps.get_model("contenttypes", "ContentType").objects.get_for_model(obj)
