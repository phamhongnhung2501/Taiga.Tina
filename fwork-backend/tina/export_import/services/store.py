...
from tina.projects.history.services import make_key_from_model_object, take_snapshot
from tina.projects.models import Membership
from tina.projects.references import sequences as seq
from tina.projects.references import models as refs
from tina.projects.userstories.models import RolePoints
from tina.projects.services import find_invited_user
from tina.timeline.service import build_project_namespace
from tina.users import services as users_service
...
def _validate_if_owner_have_enought_space_to_this_project(owner, data):
    ...
    if not enough_slots:
        raise err.TinaImportError(error_message, None)
...
def _create_project_object(data):
    ...
    if not project_validator:
        ...
        raise err.TinaImportError(_("error importing project data"), None, errors=errors)
...
def _populate_project_object(project, data):
    def check_if_there_is_some_error(message=_("error importing project data"), project=None):
        errors = get_errors(clear=True)
        if errors:
            raise err.TinaImportError(message, project, errors=errors)
...
def store_project_from_dict(data, owner=None):
    ...
    except err.TinaImportError:
        # raise known inport errors
        raise
    except Exception as e:
        # raise unknown errors as import error
        raise err.TinaImportError(_("unexpected error importing project"), project)
