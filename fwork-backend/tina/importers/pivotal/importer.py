...
from tina.projects.references.models import recalc_reference_counter
from tina.projects.models import Project, ProjectTemplate, Membership, Points
from tina.projects.userstories.models import UserStory, RolePoints
from tina.projects.tasks.models import Task
from tina.projects.milestones.models import Milestone
from tina.projects.epics.models import Epic, RelatedUserStory
...
class PivotalImporter:
    ...
    def _import_epics_data(self, project_data, project, options):
        ...
        for epic in project_data['epics']:
            ...
            Epic.objects.filter(id=Tina_epic.id).update(
                ref=epic['id'],
            ...
            for watcher in epic['follower_ids']:
                watcher_user = users_bindings.get(watcher, None)
                if watcher_user:
                    Tina_epic.add_watcher(watcher_user)

            take_snapshot(Tina_epic, comment="", user=None, delete=False)
            self._import_comments(project_data, Tina_epic, epic, options)
            self._import_epic_activity(project_data, Tina_epic, epic, options)
            counter += 1
    ...
    def _import_epic_activity(self, project_data, Tina_epic, epic, options):
        offset = 0
        while True:
            activities = self._client.get(
            ...
            offset += 300
            for activity in activities['data']:
                self._import_activity(Tina_epic, activity, options)
    ...
    def _transform_activity_data(self, obj, activity, options):
        ...
        Tina_user = users_bindings.get(activity.get('performed_by', {}).get('id', None), None)
        if Tina_user:
