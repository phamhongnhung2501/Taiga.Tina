...
from tina.projects.models import Project, ProjectTemplate, Points
        ...
        for issue_type in types:
            ...
            while True:
                ...
                for issue in issues['issues']:
                    ...
                    self._import_to_custom_fields(Tina_issue, issue, options)

                    Tina_issue.ref = issue['key'].split("-")[1]
                    ...
                    for subtask in issue['fields']['subtasks']:
                        print("WARNING: Ignoring subtask {} because parent isn't a User Story".format(subtask['key']))
                    self._import_comments(Tina_issue, issue, options)
                    self._import_attachments(Tina_issue, issue, options)
                    self._import_changelog(project, Tina_issue, issue, options)
                    counter += 1
