...
from tina.projects.models import Points
from tina.projects.userstories.models import UserStory
from tina.projects.tasks.models import Task
from tina.projects.issues.models import Issue
from tina.projects.milestones.models import Milestone
from tina.projects.epics.models import Epic
from tina.projects.attachments.models import Attachment
from tina.projects.history.services import take_snapshot
from tina.projects.history.services import (make_diff_from_dicts,
                                            make_diff_values,
                                            make_key_from_model_object,
                                            get_typename_for_model_class,
                                            FrozenDiff)
from tina.projects.custom_attributes.models import (UserStoryCustomAttribute,
                                                    TaskCustomAttribute,
                                                    IssueCustomAttribute,
                                                    EpicCustomAttribute)
from tina.projects.history.models import HistoryEntry
from tina.projects.history.choices import HistoryType
from tina.mdrender.service import render as mdrender
from tina.importers import exceptions
...
class JiraImporterCommon:
            ...
            if custom_field['custom']:
                if custom_field['schema']['custom'] == "com.pyxis.greenhopper.jira:gh-sprint":
                    ...
                    custom_fields.append({
                        ...
                        "Tina_field_name": custom_field['name'][:64],
                    })
    ...
    def _import_to_custom_fields(self, obj, issue, options):
        ...
        for custom_field in self.custom_fields:
            ...
            if data:
                Tina_field = custom_att_manager.get(name=custom_field['Tina_field_name'])
                custom_attributes_values[Tina_field.id] = data
    ...
    def _transform_history_data(self, project, obj, history, options):
        ...
        Tina_user = users_bindings.get(history.get('author', {}).get('key', None), None)
        if Tina_user:
            user = {"pk": Tina_user.id, "name": Tina_user.get_full_name()}
        ...
        for history_item in history['items']:
            if history_item['field'] == "Attachment":
                ...
                if isinstance(obj, Task):
                    field_obj = obj.project.taskcustomattributes.get(name=custom_field['Tina_field_name'])
                elif isinstance(obj, UserStory):
                    field_obj = obj.project.userstorycustomattributes.get(name=custom_field['Tina_field_name'])
                elif isinstance(obj, Issue):
                    field_obj = obj.project.issuecustomattributes.get(name=custom_field['Tina_field_name'])
                elif isinstance(obj, Epic):
                    field_obj = obj.project.epiccustomattributes.get(name=custom_field['Tina_field_name'])

                result['change_old']["custom_attributes"] = [{
                    "name": custom_field['Tina_field_name'],
