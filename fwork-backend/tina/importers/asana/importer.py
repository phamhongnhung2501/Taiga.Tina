...
from tina.projects.models import Project, ProjectTemplate
...
from tina.projects.custom_attributes.models import UserStoryCustomAttribute, TaskCustomAttribute
...
from tina.importers import services as import_service
...
class AsanaImporter:
    ...
    def _import_project_data(self, project, options):
        ...
        Tina_project = Project.objects.create(
    ...
    def _import_user_stories_data(self, Tina_project, project, options):
        users_bindings = options.get('users_bindings', {})
        ...
        due_date_field = Tina_project.userstorycustomattributes.first()
        ...
        for task in tasks:
            ...
            us = UserStory.objects.create(
                ...
                status=Tina_project.us_statuses.get(slug="closed" if task['completed'] else "open"),
            ...
            for subtask in subtasks:
                self._import_task_data(Tina_project, us, project, subtask, options)
    ...
    def _import_task_data(self, Tina_project, us, assana_project, task, options):
        users_bindings = options.get('users_bindings', {})
        tags = []
        for tag in task['tags']:
            tags.append(tag['name'].lower())
        due_date_field = Tina_project.taskcustomattributes.first()
        ...
        Tina_task = Task.objects.create(
            project=Tina_project,
            user_story=us,
            owner=self._user,
            assigned_to=assigned_to,
            status=Tina_project.task_statuses.get(slug="closed" if task['completed'] else "open"),
            us_order=task['id'],
        ...
        if task['due_on']:
            Tina_task.custom_attributes_values.attributes_values = {due_date_field.id: task['due_on']}
            Tina_task.custom_attributes_values.save()

        for follower in task['followers']:
            follower_user = users_bindings.get(follower['id'], None)
            if follower_user is not None:
                Tina_task.add_watcher(follower_user)

        Task.objects.filter(id=Tina_task.id).update(
            modified_date=task['modified_at'],
            created_date=task['created_at']
        )
        ...
        for subtask in subtasks:
            self._import_task_data(Tina_project, us, assana_project, subtask, options)

        take_snapshot(Tina_task, comment="", user=None, delete=False)
        self._import_history(Tina_task, task, options)
        self._import_attachments(Tina_task, task, options)
