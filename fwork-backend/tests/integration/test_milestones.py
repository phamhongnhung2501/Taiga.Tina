...
def test_list_milestones_Tina_info_headers(client):
    user = f.UserFactory.create()
    project = f.ProjectFactory.create(owner=user)
    role = f.RoleFactory.create(project=project)
    f.MembershipFactory.create(project=project, user=user, role=role, is_admin=True)
    ...
    assert "tina-info-total-closed-milestones" in response1["access-control-expose-headers"]
    assert "tina-info-total-opened-milestones" in response1["access-control-expose-headers"]
    assert response1.has_header("Tina-Info-Total-Closed-Milestones") == False
    assert response1.has_header("Tina-Info-Total-Opened-Milestones") == False

    assert response2.status_code == 200
    assert "tina-info-total-closed-milestones" in response2["access-control-expose-headers"]
    assert "tina-info-total-opened-milestones" in response2["access-control-expose-headers"]
    assert response2.has_header("Tina-Info-Total-Closed-Milestones") == True
    assert response2.has_header("Tina-Info-Total-Opened-Milestones") == True
    assert response2["tina-info-total-closed-milestones"] == "3"
    assert response2["tina-info-total-opened-milestones"] == "1"
