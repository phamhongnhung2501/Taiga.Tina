...
from tina.export_import import services
from tina.export_import.exceptions import  TinaImportError
from tina.projects.models import Project, Membership
...
def test_valid_project_import_without_extra_data(client):
    user = f.UserFactory.create()
    user_watching = f.UserFactory.create(email="testing@tina.io")
    client.login(user)
...
##################################################################
## tes tina.export_import.services.store_project_from_dict
##################################################################
...
def test_services_store_project_from_dict_with_no_projects_slots_available(client):
    ...
    with pytest.raises(TinaImportError) as excinfo:
