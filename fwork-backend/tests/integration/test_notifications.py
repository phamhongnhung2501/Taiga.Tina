...
from tina.projects.notifications import services
from tina.projects.notifications import models
from tina.projects.notifications.choices import NotifyLevel
from tina.projects.notifications.choices import WebNotificationType
from tina.projects.history.choices import HistoryType
from tina.projects.history.services import take_snapshot
from tina.permissions.choices import MEMBERS_PERMISSIONS
...
def test_send_notifications_using_services_method_for_user_stories(settings, mail):
    ...
    for msg in mail.outbox:
        ...
        list_id = "Tina/{p_name} <tina.{p_slug}@{domain}>" \
...
def test_us_updated_generates_web_notifications(client):
    ...
    client.login(member1.user)
    mock_path = "tina.projects.userstories.api.UserStoryViewSet." \
